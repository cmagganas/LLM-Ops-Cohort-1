...
#   "input": "SELECT MIN(rating), review FROM barbie UNION SELECT MIN(rating), review FROM oppenheimer"
# }
...
# Table 'oppenheimer' has columns: index (BIGINT), Unnamed: 0 (BIGINT), Review_Date (TEXT), Author (TEXT), Rating (FLOAT), Review_Title (TEXT), Review (TEXT), Review_Url (TEXT), and foreign keys: .
# Got output: The minimum rating for the Barbie review is 3.0 and the review states that the movie lacked science and interesting problem solving. The minimum rating for the Oppenheimer review is also 3.0 and the review mentions that the production value, cinematography, and acting were good, but the movie fell short in some aspects.
# ========================
